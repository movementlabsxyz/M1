...
use avalanche_types::{ids, jsonrpc::client::info as avalanche_sdk_info, subnet as avasubnet};
...
async fn e2e() {
    ...
    let vm_id = Path::new(&vm_plugin_path)
        ...
        .unwrap()
        .to_string();
    let vm_id = avasubnet::vm_name_to_id(&vm_id).unwrap();
    ...
    let genesis = subnet::genesis::Genesis {
